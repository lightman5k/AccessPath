    ...
    case "discussion":
      return (
        <svg aria-hidden="true" className={className} fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth={stroke} viewBox="0 0 24 24">
          ...
          <path d="M6 12h8" />
          <path d="M6 16h5" />
          <path d="M5 4h14a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2h-6l-4 4v-4H5a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2z" />
        </svg>
      );
    case "insights":
      return (
...
export default function AppShellClient({
  ...
  const routeNavItems = useMemo(
    () =>
      navSections
        ...
        .filter((item): item is RouteNavItem => 
          typeof item.href === "string" && (!item.requiresAdmin || session.role === "admin")),
    [session.role],
  );
  ...
  const acknowledgeLockedFeature = () => {
    if (!lockedFeature) return;
    const requirement = featureRequirements[lockedFeature];
    setLockedFeature(null);
    setToastMessage(
      requirement.plan
        ? `${requirement.plan.charAt(0).toUpperCase()}${requirement.plan.slice(1)} access is managed by your signed-in workspace.`
        : `${requirement.role === "admin" ? "Admin" : "Agent"} access is managed by your signed-in workspace.`,
    );
    setToastOpen(true);
  };

  const planValue = session.plan ?? "free";
  const roleValue = session.role ?? "admin";
  const planLabel = planValue.charAt(0).toUpperCase() + planValue.slice(1);
  const roleLabel = roleValue === "admin" ? "Admin" : "Agent";
  const roleInitials = roleValue === "admin" ? "AD" : "AG";
  ...
  const renderSidebarItem = (item: NavItem) => {
    ...
    if (active) {
      return (
        <span
          aria-current="page"
          className={`${commonClasses} bg-sky-600 text-white shadow-[0_10px_24px_rgba(2,132,199,0.28)] cursor-default`}
          key={item.label}
        >
          <span className="flex items-center gap-2.5">
            <span className="flex h-7 w-7 items-center justify-center rounded-lg bg-white/16 text-white ring-1 ring-white/20">
              <SidebarIcon active={true} className="h-4 w-4" name={item.icon} />
            </span>
            <span>{item.label}</span>
          </span>
          <svg
    ...
    return (
      <Link
        className={`${commonClasses} text-slate-700 hover:bg-white/85`}
      ...
      >
        <span className="flex items-center gap-2.5">
          <span className="flex h-7 w-7 items-center justify-center rounded-lg bg-white/90 text-slate-500 shadow-sm ring-1 ring-slate-200">
            <SidebarIcon active={false} className="h-4 w-4" name={item.icon} />
          </span>
          <span>{item.label}</span>
        </span>
      </Link>
    ...
    <div className="min-h-screen bg-gray-50 text-gray-900">
                ...
                {navSections.map((section) => (
                  <div key={section.title}>
                    ...
                    <nav className="mt-2 space-y-0.5">
                      {section.items
                        .filter((item) => !item.requiresAdmin || session.role === "admin")
                ...
                <button
                  ...
                  onClick={async () => {
                    try {
                      ...
                      // The local demo state still needs clearing even if the auth route is unavailable.
                    }
      ...
      <Modal
        footer={
          <button
            ...
            onClick={acknowledgeLockedFeature}
            type="button"
          >
            Understood
          </button>
      ...
      >
        ...
        <p className="mt-3 text-sm text-gray-500">
          Current workspace access: {planLabel} plan, {roleLabel} role.
        </p>
      </Modal>
    </div>
  );
