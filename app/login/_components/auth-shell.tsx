...
export function FieldIcon({ kind }: { kind: "email" | "lock" | "user" | "building" }) {
  if (kind === "email") {
    return (
      <svg
        aria-hidden="true"
    ...
    <main className="min-h-screen bg-white text-slate-900">
      <div className="grid min-h-screen lg:grid-cols-[minmax(420px,0.95fr)_minmax(520px,1.05fr)]">
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="mt-2 text-sm leading-7 text-slate-600">
                      &quot;AccessPath streamlined my bakery&apos;s logistics while keeping our vendor
        ...
        <section className="flex items-center justify-center px-8 py-12 lg:px-12">
          <div className="w-full max-w-md">
            <div>
              <p className="text-sm font-medium text-sky-700">{formEyebrow}</p>
              <h2 className="mt-3 text-4xl font-semibold tracking-tight text-slate-950">{formTitle}</h2>
              <p className="mt-4 text-base leading-8 text-slate-600">{formDescription}</p>
            </div>
            ...
            <p className="mt-6 text-sm leading-7 text-slate-500">{legalCopy}</p>
          </div>
        </section>
      </div>
      ...
      <footer className="border-t border-slate-200 bg-white">
        <div className="mx-auto flex max-w-7xl flex-col gap-3 px-8 py-5 text-sm text-slate-500 md:flex-row md:items-center md:justify-between">
          <p>&copy; 2026 AccessPath for Business. AI-Powered Excellence.</p>
